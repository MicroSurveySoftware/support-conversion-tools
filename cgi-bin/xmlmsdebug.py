...
#Find all necessary Header Information:
    ...
    for headerunits in root.getiterator('{http://www.landxml.org/schema/LandXML-1.2}Metric'):
...
#Find all CgPoints:
    ...
    for i in range(len(lines) - 1):
            ...
            if "</CgPoint>" in curline:
                ...
                code = ""
                if 'code="' in curline:
                    code = curline[curline.find('code=')+6:curline.find('" t')]
                if "-" in name:
                    hyphenwarn = 1
...
#Find All Setup Records:
    for setup in root.getiterator('{http://www.landxml.org/schema/LandXML-1.2}InstrumentSetup'):
    ...
    for rawobs in root.getiterator('{http://www.landxml.org/schema/LandXML-1.2}RawObservation'):
        comment = ""
        ...
        TargetPoint = rawobs.find('{http://www.landxml.org/schema/LandXML-1.2}TargetPoint')
    ...
    for setup2 in root.getiterator('{http://www.landxml.org/schema/LandXML-1.2}InstrumentSetup'):
        ...
        for rawobs2 in root.getiterator('{http://www.landxml.org/schema/LandXML-1.2}RawObservation'):
            setupID = rawobs2.attrib["setupID"]
            TargetPoint = rawobs2.find('{http://www.landxml.org/schema/LandXML-1.2}TargetPoint')
